# 全新红帽认证／Linux云计算架构师／RHCE／RHCSA必备课_可零基础入门的教程 - P2：1.01-Linux命令基础 - 达内教育 - BV19U4y1b7qQ

嗯，那下面我们要讲这个linux命令基础。嗯，这一块的要解决的问题啊，就是帮助大家。你在对命令了解不是特别多的情况下，那有一些命令是我们需要去了解的，需要去掌握的。你掌握的这些命令呢。

那我们会面在学这些知识的时候才能够更加顺利。Yeah。嗯，初步规划的我们大概包括这么几个部分吧。Yes。lininux命令基础这一块啊。Okay。那第一个就是关于我们lininux。

命令行去控制我们操作系统。它的一些基础的一些概念啊。命令一行概述。啊，该数这一块的话呢，你可以包括它的命令行那些格式是吧？啊，快捷键。这可能有些同学听过啊，那没关系，你就当复习一下了。那第二个的话呢。

就是常用的一些命令。嗯，常用的命令的话呢，我们主要给大家讲解这么几个，嗯，像LS呀是吧CD呀啊PWD呀。这个是要了解我们lininux系统的文档目录结构啊，我们需要去了解的。唉。

那再一个呢就是在我们linux系统里边创建目录需要一个mateDIR啊，然后呢创建文件的。查看文件的。Cat。nice是吧。然后复制文档的CP啊copy。删除文档RM。啊，还有改名移动文档MV对吧？

这几个啊大家必须会的吧，是吧？这很基术很基础的一些操作啊。嗯。Yes。那第三个的话呢，技术命令这一块呃，还希望大家了解我们。命令的帮助。嗯，为啥要学这个呢？因为我们在考红包认证的过程当中。

有很多东西你可能。并不一定能够全都记下来。那当你记不住一些东西的时候，一定要学会用帮助。对吧你要用学会用帮助的话呢，嗯那很多资料你一查就能查出来，不需要全都记住。这个帮助指的是我们红帽系统里边啊。

他为我们提供的这些帮助，不需要上互联网系统里面自带的啊。这个一般有一个叫手册的一个工具，简写的就是1个MAN啊，非常强大的那这个你要知道怎么去看，怎么去找资料是吧？我们后面有很多操作呢。

你都要学会用这种方法。😊，嗯，这种方法呢，你在考试的时候，你随便用，不算你做弊哈，在正大光明的看。😊，因为你谁能说我们工作的工作的时候，你不让我看资料啊，然后谁记性也不是那么好的，是吧？

有很能东西我顺手就能找出来的那我为啥不查呀？😡，你做做能把问题解决啊，就好了。😊，啊，这是命令帮助，这是一块。那后边还有一个呢是VIM编辑器啊，VIM编辑器。嗯，这个编辑器呢在我们最小化的安装里面。

可能没有VIM只有1个VI。但是大家用的时候，这两个编辑器用法差别不是特别大。所以这是叫VM编辑器啊。呃，那基本上的话呢，我们lininux命令基础这一块大概就是这么多，对吧？一会我们分别来看一下。

讲完这一块，我们再给大家去讲那个服务控制和安全开关啊。嗯。第一个命令行概述。呃，命令行概述的话，其实。这要写起来也挺多的啊，所以不写了，我就说吧。呃，命令行开始数，你首先要知道什么是命令行是吧？😊。

什么是命令行啊，啥意思？我们说咪令啥意思是吧？啊，你家养一条小狗，人家说来过来握个手。😡，对吧他如果能听懂你说的这句话，他是不是就伸个爪子过来给你握一下呀，对不对？😡，那可能你刚新新认识一个女朋友。

你带回家去，女朋友跟他说是过来的大黄能握个熟。😊，可能你那条狗就听不明白，对吧？因为是你跟他说的那个语调啊，声音啊都不一样，他不一定就理会这句话。😡，所以呢我们控制lininux主机。

那这个lininux主机对我们管理员来说，我们得让他听话呀，它就是我们养着一条小狗一样啊，是不是这样的是吧？那我们作为用户，我们作为管理员，就得让lininux主机听我们的话。😊。

所以我们需要给他去发命令行。那怎么发呢？其实就是在我们这个命令终端啊，你能输入这些指令的这个终端去敲一段代码。你包括我们刚才敲的这个SSH，这些都是命令啊。所以我们说的命令行其实就是有管理员。Yeah。

输入的一串。字符是吧。这一双制符呢，它的作用是呢用来干嘛的？啊。是一套什么样的字符啊，是吧？是用来完成。某一个系统任务的。你输一个字符，输一串字符，肯定是要干活的。你不干活，你不不啥事也不干。

那这不是不是一个有意义的命令嘛，是吧？你管理员输入的一串用来完成某某系统任务的啊，或者不一定要系统任务是吧？有完成某一个任务的。😡，管理员输入的一串用来完成某某任务的制符啊，输入的形式。就是你按回色键。

就是个and键。提交。那么你按按键提交之前的这一串字符，你就可以把它当成一个命令行。对吧这就是一个命令行。比方说嗯我们前面说的各种啊。不。我们要看IP地址，最小化安装的系统呢。

你可以IP address list。对吧可以用这种方式。淘网卡不叫这个名字是吧？叫EN啊EMPES0，那么这就是一串命令。那我们在输入这一串字符的时候，如果他能认识，能听懂。

那说明这个lininux系统呢。他就能够帮我们做一些事。如果他听不懂，他可能就会报错。那说明你敲的这条命令有问题，你就得去纠正啊是吧？你像我们上面敲的这条命令啊，IPADDlist后面给你1个EH0。

然后他就告诉我啊，你这个主机上没有ETH0这块网卡。😊，不行吧，是吧？啊，那没有这个网卡，我就想知道到底有哪颗哪几个网卡呢？我就把后背E天就能给去掉了，只敲了1个IB address list。😊。

他告诉我下边哎。有一个回还接口LO然后有一个第二个接口叫EMPES0是吧？这是一块以太网卡，然后它下边IP地址。那这个命令的作用呢，其实就是用来看IB接口啊，它相关的一个地址信息。Yeah。啊。

那为什么我们的inux系统你发一个指令，它能听懂呢？😡，那我要敲个别的，他为什么就听不懂呢？😡，是吧所以那这一块的话呢，大家还需要知道一个什么东西啊。😊，还需要知造一个叫解释器啊，有些概念啊。

刚才我们讲到的第一个案件叫命令行，你要知道命令行是啥，那解释器又是啥呢？这个指的是lininux系统中的。一个程序。这个程序是一个特殊的程序。怎么个特殊的特殊法。因为这个程序呢。它是用来翻译。

或者说叫解释。管理员提交的命令行的啊。有一个这样的一个特殊程序。这个特殊程序就是我们的解释器。如果我们的lininux操作系统没有这个解释器。那我们管理员啥也干不了。啊，为啥呢？因为你敲一个明令。

没人理你，对吧？就好像我们养了条小狗，那那小狗的耳朵可能被个啥东西堵住了，听不见。😊，那你给他说话，他不理你，对不对？😡，那不行，所以我们linux系统呢，其实任何操作系统都需要有一个解释器。啊。

都需要一个解释器。这个特殊程序呢通常就是。有一个概念啊。他有个概念，一般叫做笑。啊，这就是我们所说的笑。那为什么线要跟解释器挂上钩了呢？因为它的外它的那个英文的原意啊叫外壳，啊，为啥叫外壳啊？它是。😊。

它是包在我们在包在我们的lininux内核程序啊，就是操作系统的最核心的那个程序外边的一层壳啊。这是包在linux内核外边的。一层壳，所以叫外壳程序。这个外壳程序它的作用就是用来为我们用户翻译命令。
...
来了解我们系统里边有哪些文件。那默认情况下呢是在这一个加的一个图标是吧？叫home，这是你用户的总目录。但是如果你想了解这个系统里面所有的文件，应该找最底下有一个otherloc。就其他位置啊。
...
所以大家知道为什么我们。😡，搞ID运维的要喜欢用命令行，因为你用图形界面太麻烦了。我命令行敲个LS空格，斜杠顶多敲4个字符。
...
啊，当然啊我们讲的这个目录结构了，那大家也了解一下我们的。linux目录结构里边常见的一些目录，它是用来干嘛的，是吧？常见的一级。葫芦。啊，一级公路。呃，然后呢lininux系统的这目录，它是分层次的。
...
这种层积结构呢你不需要用肉眼去分辨，你直接敲就行。😡，你要找bot目录下面打1个BO往下tableable一下。是吧这涉及到一个快捷键。😊，呃，你往下再找一个目录，就是gb to再table一下。
...
你这不有个home吗？我用管理员进来，这也有也是有一个home的。😊，那上面一个标标志画了一个房子的一个图片是吧？啊，当然啊，管理员root这个用户，他的加目录。
...
media这个是用来做自动挂载的啊。可以对比一下。放到这个地方，那MNT呢是由管理员。手动挂着一些外部设备的目录哎，他建议你放这啊，但你管理员有这个管理员权限，你要你你到底挂载不到。
...
那如果选项有多个呢，都是一个字母这种的，我们可以把它简写。啊，管理员就要想各种办法，怎么简单怎么来啊，可以简写成杠LH这个其实就相当于杠L空格杠H。效果是一样的啊效果是一样的，你看它会分别列出。
