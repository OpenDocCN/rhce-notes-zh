# RHCE8.0视频教程【45课时】 - P37：20200510-RHCE-02_recv - 六竹书生6682 - BV1su4y1Z7sJ
...
来远端的哪个考不到本地的哪个，但是remove source我没有试哈，这边的话呢我们就使用这两个，就使用这两个，然后的话呢刚才不是创建了一个脚本吗，我们的话呢将temp下面的dt。s h发送过去。

那发送的话呢就是answerable对谁呢，web一使用copy命令里面的参数是怎么样呢，本地的哪个文件呢，temp目录下面的离at s h发送去哪里呢，比如说发送给远端的temp目录，等一下。
...
下面这边的话呢我们还有比较常用的是什么呢，有时候我们是不是希望对嗯，s1 linux去进行一个关闭，对吧，我们先来看一下它远端sa linux，linux的一个信息是怎么样的，get in force。

比如说回车，这边的话呢它是一个开启状态，如果说我们想要去关闭的话呢，方法第一个是不是先可以把它设置成叫做嗯，临时关闭，是不是就是这个set in force 0就可以了，如果说想要去永久关闭呢。

所有的主机我都想要去永久关闭，那这边的话呢，是不是就要去把我们的sa linux的配置文件信息，去进行一个发送啊，去进行一个发送啊对吧，那这边的话呢第一步我给你们去写一下。

虽然这个过程你们可能觉得会比较简单一点，还是给你们去写一下，这里，首先第一步，我们是不是要去编辑我们的se linux文件，编辑好了之后呢，发送给远端主机吧，发送过去之后呢。

我们可以去检查一下是不是真的在远端了，对不对，是不是真的在远端了，然后下面这边的话呢，我们还可以就是说嗯去给设备呢进行一个重启，进行一个检验嘛，这边重启的话，那我就不去进行一个重启了，那这边就这样子吗。

copy文件的话呢，是不是在一个etc，试试configure sa linux，比如说拷贝的temp目录，下面来，我们的话呢去编辑tesa linux这个文件，它里面的话呢信息很多。

是不是只要把这个叫做disabled给改掉，啧啧啧啧啧，改完了之后呢，我们去保存退出，对不对，然后接下去的话呢就去发送了嘛，unable，他这边的话呢，你看格式src，我本地是什么文件呢。

s linux文件发送到远端的哪里去呢，是不是要去发送到远端的这个etc se linux下面去啊，sa linux下不去，如果说我直接这样子去发送过去的话，会有什么样的一个问题。

你们想一下或者什么样的一个问题，l s l l减l etc，this is config sa linux，这个的话呢是不是一个叫做5月3号，5月3号可能去进行的一个编辑嘛。

如果说我直接这样子发送过去的话呢，是不是会把原来的一个文件去进行一个覆盖，我们的话呢刚才在看的时候，是不是还有个参数叫做back up，这个的话呢叫做对远端文件进行备份，我们等一下去看一下，效果好吧。

这边我先写，先跟上这个yes，稍等一下，我先把这个效果给大家去截图一下，等一下，他如果不在一行的话，我就会特别难受，所以有时候喜欢把这个格子拉的大一点，然后你看我这边的话呢发送过去了，发送过去了。

然后这边我们再来看一下slinux，然后呢信号所有和sa linux相关的，这里的话呢，哎我这时间是不是不准啊，诶5月10号对的呀，哦估计是这个文件，它的一个日期是怎么样子的，诶5月10号呀。

他这边日期有问题吗，5月10号呀，这边好像在发送过来的时候，显示的信息稍微有点问题啊，他这边是怎么样呢，其实一开始的时候是没有这个文件的，没有这个叫做什么1293这个信息的，或者我现在这边的话呢。

先把它给删掉，temp哦不对，etc c config sa linux 123几吗，然后这边的话呢我再来给你们看一下，现在就一个这个文件后面加个新号哈。
...
如果想要去设置这边的，文件属性的话呢，在拷贝的时候可以去设置，比如说拷贝完了之后呢，那我们是不是可以这样子，unable m shell，干嘛呢，全剧mode 7777对谁呢，temp下面的v一吗。
...
我们在使用我们的叫做file的一个模块，那他这里的话呢是不是就应该跟上的，是一个叫做fire的一个信息，然后后面呢这边就要去跟上路径，我的话呢希望在tap目录下，比如说创建一个dead 2。
...
嗯我看一下他这个using fire，我看一下，难道是可以删除，这里确实是删除了，我这边再来touch几个吧，叫做fire b一到五，诶哦哦，我看一下这边的话呢去加上一个双引号行不行，我。
