# Linux最全RHCSA+RHCE培训教程合集，小白入门必备！ - P39：红帽RHCE-3.编写脚本、脚本执行方式 - -Book思议8 - BV1ZV4y1v7c8
...
那enable呢也得写一个enable，等于一启用仓库不仅的签名，是不是啊，那接下来这个是不是可以粘贴到这里面的，是不是当我们是写脚本啊，这种东西就是为了给大家演示，这边我不保存是吧，好那现在问题来了。
...
echo仓库的这些配置啊，我可以这样echo中括号，仓库的名称放到一点TT里面，一点TT里面是不是可以，那这个一点ta t是不是就仓库的名字，接下来我再EQ一个仓库的描述local。

然后也给他放到这个一点TT里面，我再看一点TTP好，原先的没了是吧，哼原先EQ的没了，所以怎么办，还得用两个大于号，因为一一个大于号是覆盖，两个大于号是追加，是不是啊，所以这时候我们得先得先这样。

先把这里的内容给它清空一下，怎么清空呢，叫echo直接这样一点TT可不可以呢，可以那里边的内容就空了，为什么空了呢，为什么会空呢，因为我什么都没echo，我什么都没有echo，那这里面就是一个空行。

知道吗，一个空行echo是啥意思嗯，echo是一个输出的命令，这些前面不都讲过吗，前面都讲过哈，在HCC笔记里面有好，然后那我还就得这样呗，echo是不是啊，比如说echo一个仓库的名称，那就是中括号。

然后追加到哪呢，当你第一次你不需要追加，如果是第一行的话，你可以这样覆盖也无所谓，因为毕竟是第一行嘛，嗯当然一般情况下我们对于这种文件来讲，它里边有空行也不行，有空行也不行，所以把文件给它删掉。

我们再重新演示一下，echo好，仓库的名称，local放到一点TT好，这个一点TT里面是不是有仓库的名字了，接下来echo仓库的描述local追加到一点TT里面，那这时候一点TT里面就两行了啊。
