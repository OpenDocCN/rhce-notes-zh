# 新盟教育-Linux运维RHCSA+RHC培训教程视频合集，全网最新最全最详细！ - P2：红帽RHCSA-2.VMware安装CentOS系统 - 广厦千万- - BV1up4y1w729
...
行，那我们说说这个虚拟机软件，你给它下载到你电脑里边以后呢，就是就是这个哈就这个图标看到叫veware worker station。😊，然后呢，双击打开它。
...
还有oracle的，还有红帽read hat的看到了非常多吧，是不是啊？啊这里非常多哈，O但是你看包括乌班图的都有啊。那我们选哪个呢？我们选择sintoS的系统往上滑在这，然后找到对应的版本。

你看那sinto啊在上面，stos分为567，我们选择这个7版本，我一会儿会给你们介绍啊，这个现在这个操作系统的版本。😊，操作系统的版本。介绍介绍什么呢？介绍这个。同帽的。跟这个圣透S。

它分为这个567版本。红帽也是567567，然后sinS呢也是567版本。Okay。然后这个位置呢选好的话呢，我们直接选择下一步啊，然后在这里边啊叫命名虚拟机，这就是让我们给这个虚拟机取个名。

这就是左边的名字哈。这个名字呢你随便起啊，叫什么都行，中文英文都可以。你比如说我。😊，我就让他叫什么叫。37。m。然后系统班学习啊，我我让叫教学用机。教学机。可以吧，然后后面呢再指定一个它的IP地址啊。

IP地址现在还没有配，我们后期再配哈。IP呢我让它变成多少呢？变成。😊，啊，90啊，我们后期会会给他配个IP啊，IP地址的最后一位就给它配成90啊，IP多少，我们一会儿再说好吧啊。

你们可以先按照我这样去什么呢？去命名。然后接下来安装位置，那安装位置的话呢，这个你可以自由选择了哈，在预览里边，你觉得你电脑里边看你的哪块盘比较。😊，富裕啊，就空间比较充裕，看到了吧？呃，此电脑。

你这里边你觉得你你的C盘充裕啊，还是你的别的盘比较充裕啊是吧，你就给它安装在哪个盘上面就可以了。然后如果C盘，你的空间如果已经怎么说呢？占用比较高的话啊，对哈，最好是放到固态里边，因为它快嗯。😊。

所以呢呃。你就尽量吧啊尽量能放到C盘就放到C盘。为什么呢？因为C盘啊就是如果你电脑是固态硬盘，你的C盘肯定是固态的啊。如果空间充裕的话，放在C盘。如果空间不不充裕呢，就别放到C盘了，毕竟C盘。

在windows里边，C盘就是你的系统盘啊，就是系统在启动的时候啊，系统在运行的时候啊，它的相关的重要文件都在这个C盘里面。所以如果你的C盘空间太臃肿的话，会影响你系统的运行速度。啊，所以看情况啊。

你们就看情况安，我呢就安装我的安装到我的C盘。你看我的C盘。我还可用空间还有1175个G呢啊，所以我随便随便用哈OK那默认呢就在C盘了，看到吗？C盘这个路径，然后选择下一步。😊，OK然后指定磁盘容量啊。

这个磁盘容量默认是。他给我们分配了20个G的空间，这20个G的空间是从哪来的呢？注意哈，它是虚拟机，你看它给我们虚拟一个磁盘，然后呢这个磁盘是20个G的空间。😊，那这个虚拟的磁盘空间。

那最终它得从我的真机啊，就是我的windows电脑。的空间去干嘛呢？去划分。出20个G的空间给这个虚拟机续用。能理解我说的意思吧？你比如说我这个我的电脑总共100个G的空间啊。

那这时候呢我给虚拟机呢分配了20个G。😊，啊，那他就会从我的100个G里面去划走20个G分配给虚拟机能理了吧。那我的电脑还剩多少呢？是不是还剩80个G啊？😊，没错吧啊，是这样子的哈。😊，但是我告诉你。

这个空间呢，它不是说一次性就划走，你给他800，没事哈，它不是一次性就全都把空间分配给虚拟机哈，它是这样子的，就是随着不断的使用，然后呢再1。1点的给这个虚拟机分配空间。

你比如虚拟机运行总共啊需要占用两个G的什么呢？磁盘空间。那这个时候哎我们这个真机啊，就会给它分配两个G能理解吧？啊，虚拟机如果说随着不断的使用啊，需要3个G，那就给它分配3个G。

它不是说一次性把20个G全都给它。😊，啊，就是随着不断的使用啊，去干嘛呢？啊，去给他分配这个使用量，是这么一回事。所以呢呃你们也不要担心，你说我我给他分配100个G，那是候我真机电脑就没有空间用了。

还不是。😡，那这个空间其实是什么呢？这个空间是最多哎，你可以使用我。20个G空间。啊，也就是我最多会给你分配20个G。那然后随着不断的使用呢，1。1点分配给你能裂了吧。啊。

所以这个你不要担心分大分小都无所谓，在这个位置。然后下边呢。😊，啊，你到底是将虚拟磁盘拆分为啊多个文件盒儿。给他。啊，存储为单个文件。那这个有什么区别吗？他就告诉你了，你看如果拆分磁盘以后呢。

可以更轻松的在计算机之间移动虚拟机。啊，但可能会降低大容量磁盘的性能啊，这个你不用去管了哈，你就存储为单个文件就行了啊，这个我们几乎不会去移动虚拟机的，能理解吧？啊，所以选择下一步。😊。

OK然后接下来呢这个位置哈还是蛮重要的哈，叫自定硬件。😊，所以选择自定义硬件。什么叫自定义硬件啊？啊，这个时候呢我们就要涉及到虚拟机的本质了。这个虚拟机软件介绍软虚拟机软件干什么用的呀？

它是帮我们虚拟出一套完整的。硬件平台。然后呢，我们是在这个硬件。平台之上。去安装什么安装。操作系统。这个应该比较好理解吧。你比如说你从外边你花钱买个电脑回来。😡，你这个电脑里面有什么呀？

你电脑里边无非不就是一堆硬件吗？哪些硬件啊？😡，内存条是不是啊？还有什么呀？还有磁盘，像砖头一样是吧？磁盘有网卡，是不是哎硬盘。嗯，啊，到会磁盘就硬盘啊，还有什么显卡之类的。没错吧，哎，鼠标。

然后键盘是不是？这如果你从外边花钱买个电脑，你的电脑里面不就是这一堆硬件嘛？是不是就一堆硬件哈，然后呢，你是在这一堆硬件之上去干嘛呀？呃，去才能安装操作系统。因为操作系统是需要底层的硬件资源。😊。

他才能够。安装成功的。啊，因为系统最终是安装在你的磁盘里边，那你没有磁盘，你怎么安装系统啊，是不是？那你没有内存的话，你这个系统也开不开机呀，是不是啊，你没有显卡，你想你怎么看呢？是不是嗯？😡。

显卡声卡之类的，是不是？那你看啊什么网卡显卡声卡之类的，没错吧，哎，这不就一堆硬件嘛，所以虚拟机它干的事儿就是帮我们虚拟出一套完整的硬件平台出来。哎，有了硬件平台了，咱们再去干嘛呢？

再去安装系统就可以了。所以你看我们前面干的事儿，干了什么事，就是创建了一个虚拟机出来，知道吧？😊，那这个虚拟机它不就是帮我们虚拟出了一个内存条是吧？CPU。还有什么呢？啊，这是一个叫DVD啊。

这是虚拟出一个驱动啊，光驱设备哈。😊，但是呢我们一会儿再说，好吧，还有网络设备器，这叫网卡啊，帮我们虚拟出一个网卡出来了啊，USB控制器啊，就是USB接口你看虚拟出一个声卡来啊，打印机的接口啊，显示器。

😊，看到了吗？这不就虚拟他干的事吗？😡，啊，但是没有什么鼠标键盘哈，这里面。😊，好，那接下来我们就得选择一下啊，我们对这一堆硬件，你比如说内存啊，你希望这个虚拟机的内存空间是多大的内存呢？哎。

这个就蛮重要了。你看啊那虚拟机呢啊就是我们改呀，就是你的对于这个虚拟机。注意事项啊。注意事项。呃。呃，我们在这个什么呢？在学习的环境哈，学习环境，我们是通过虚拟机去安装操作系统的，安装linux的系统。

嗯，但是呢我们对于我们真机的一个内存啊，其实我还是什么呢？还是建议哈建议在16个G以上。为什么呢？因为这个虚拟机特别消耗内存，虚拟机就比较消耗内存。啊，其实也不能说虚拟机消耗内存，而什么呢？

而是本身我们的电脑内存就。😡，比较少，没错吧，你看我你知道我电脑多少内存吗？我的电脑。😡，给你们看一看哈。我的我的电脑总共8个G内存。😡，8个G的内存哈。来给你们看一下，你看我的内存就8个G，看到了吗？
...
得通过你的真机。啊，还有一个问题就是虚拟机，我想跟这个什么呢？呃我想跟我的真机之间相互通信。那这时候他们之间怎么去通信呢？哎，那这时候呢就是我所说的这叫网络模式。所以这网络模式就是用用来决定你的虚拟机。
...
去用要网络不频繁更换的环境下可以用它。那如果你的网络频繁更换，比如说个笔记本啊，来回要换网。那这时候呢。你选择NE模式NE模式呢没关系哈，虚拟机跟你的真机你不需要在这个必须得在同一个网段。
...
我们安装系统呢会这个做一些一系列的操作啊，所以这时候啊你的安装界面啊选择语言。那这时候呢默认是英文的哈那这个这无所谓，你可以。😊，选择中文啊下边哈下拉看到，这里叫中文安装界面嘛无所谓。
...
这样吧哈，我发一个网络的。视频啊。嗯，考完C就去干IE啦。哼别删是吧？😊，我们整个学习阶段，我我们用到的所有软件包都是可以在线下载哈。
...
还讲完就上传。包括我们讲课用到的课件。笔记。啊，以及相关的所有东西啊，只要课上用到的啊，都会上传。
